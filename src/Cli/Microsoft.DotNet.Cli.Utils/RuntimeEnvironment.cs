...
namespace Microsoft.DotNet.Cli.Utils
{
    ...
    internal static class RuntimeEnvironment
    {
        ...
        private static string GetOSVersion()
        {
            switch (GetOSPlatform())
            {
                ...
                case Platform.Solaris:
                    ...
                    return RuntimeInformation.OSDescription.Split(' ')[2].Split('.')[0]; // we only need the major version
                default:
                    return string.Empty;
            }
        }
        ...
        private static Platform DetermineOSPlatform()
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Create(nameof(Platform.FreeBSD))))
            {
                return Platform.FreeBSD;
            }
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Create(nameof(Platform.illumos))))
            {
                return Platform.illumos;
            }
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Create(nameof(Platform.Solaris))))
            {
